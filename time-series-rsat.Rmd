...
In the following chunk, we attempt a sort of 'sign-in' blitz. This is a crude attempt to solve ongoing connection issues between `rsat` functions and their registered *APIs*. Two NASA APIs (USGS-eros & Earthdata) reported in package documentation. However, source code shows edits in package updates and github commits. Though, the main issue seems to be the constant maintenance and reformatting of API end-points by NASA & USGS. See below some details on connection tests, make sure to change your username and passwords where suggested. More on GDAL configuration for passive computing in [gdal_cubes.md](https://github.com/seamusrobertmurphy/time-series-gdalcubes/blob/main/time-series-gdalcubes.md)
...
Most reliable function from `getSpatialData` that promtps a PAP sign-in method:
...
Derive `rtoi` filter object for bugfree functions. This spatial fileter object assigns download path for files and metadata, and allows to restore and rapid plotting even after connection breaks. NOTE: better to recall previous `rtoi` or can following chunk if previous `rtoi` was purged.
